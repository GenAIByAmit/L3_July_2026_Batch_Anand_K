...
2. In `create_agent`, what is the `system_prompt` used for?
   ...
   - B) To define the agent's role/behavior and how it should use tools
...
3. Why is `@tool` placed on a plain Python function before passing it to an agent?
   ...
   - C) It auto-generates the JSON schema (name, args, description) that the LLM sees
   - D) It caches results
...
5. How does the agent decide which tool to call for a user request?
   - A) The LLM matches the request to the tool names/descriptions in the schema
...
6. What typically happens when a model emits an invalid tool call in the graph?
   ...
   - B) The error is passed back to the model as feedback so it can recover
...
7. Built-in tools like DuckDuckGo search or Wikipedia are used to:
   ...
   - C) Give the model access to up-to-date external information
...
9. After `agent.invoke(...)`, where do you find the final assistant reply?
   - A) In `result["messages"]` as the last AIMessage
   - B) In `result["answer"]`
   - C) In `result["tool"]`
...
10. `temperature=0` on the chat model is used for:
    ...
    - B) More deterministic/reliable tool-calling behaviour
...
11. After each agent turn, the updated conversation state is kept:
    ...
    - C) In the checkpointer, keyed by `thread_id`
...
12. If you restart the Python process, which of these still has its data?
    ...
    - D) `SqliteStore` (SQLite-backed)

13. The namespace (e.g., a user id) in a Store represents:
    - A) Who owns the long-term facts — scoping by user
...
14. The checkpointer stores state under a key derived from:
    ...
    - B) The `configurable.thread_id` in the config
    - C) The tool names
    - D) The model name
...
15. The memory-tools pattern (`get_store` / `get_config`) lets the agent:
    ...
    - C) Save and recall durable facts about the user by itself
    - D) Reset the thread
...
16. Long-term memory (Store) is the right place for:
    ...
    - B) Tool definitions
    - C) The system prompt
    - D) Durable user facts (preferences, profile) that should survive across sessions

17. Short-term memory answers "what was just said"; long-term memory answers:
    - A) "what do I know about this user across sessions"
    - B) "which tools exist"
...
18. Why choose a SQLite/Postgres-backed memory instead of in-memory in production?
    ...
    - B) Data persists across restarts and can be shared by multiple processes
...
19. One agent instance serving many users works because:
    ...
    - C) Each user's conversation is isolated by its own `thread_id`
...
20. Which statement is TRUE about memory in this training?
    ...
    - D) Memory is determined by how you call the agent (`thread_id`) and the store you attach

## 3. LangGraph Fundamentals

21. Before you can invoke a LangGraph, you must first:
    - A) Call `.compile()` on the StateGraph
    - B) Call `.train()`
...
23. What does each node function receive as input?
    ...
    - C) The current shared state (dict)
    - D) The final answer
...
25. The "else" branch of a conditional edge (when there are no tool calls) typically routes:
    - A) Back to the assistant to continue the conversation, or to END
...
26. To fan out to several parallel worker nodes, you connect:
    ...
    - C) Each worker to END only
    - D) A single loop node

27. A sub-graph communicates with the parent graph via:
    - A) Files on disk
    - B) HTTP calls
    - C) State mapping — parent passes selected keys in, sub-graph returns updates
...
28. Node A returns `{"count": 5}` and node B returns `{"status": "ok"}`. The merged state:
    ...
    - D) Contains both keys

29. A good reason to use a conditional edge instead of a fixed edge:
    - A) Route differently based on runtime state (e.g., tool-call results)
...
30. `graph.invoke(...)` with a config returns:
    - A) A token stream
    - B) The final state after execution
...
31. Fan-in combines parallel results by:
    ...
    - C) Merging their state updates into the shared state
...
32. Besides reuse, sub-graphs give you:
    ...
    - D) Isolation — the sub-graph's internals stay contained

33. In a supervisor pattern, the supervisor node:
    - A) Routes the task to the right specialist agent and aggregates the result
...
34. Handing off control between agents in the supervisor pattern is done via:
    - A) HTTP redirects
    - B) State/commands that route to the next agent node
    - C) New threads
    - D) Database writes
...
35. When a sub-graph updates a key, the parent sees the change because:
    ...
    - C) The sub-graph's returned state maps back into the parent's state
...
36. A typical fan-out use case:
    ...
    - C) Storing memory
    - D) Generating multiple drafts in parallel, then merging/choosing the best

37. Why validate input inside a sub-graph rather than inline in the main graph?
    - A) Keeps the main flow clean and lets the validator be reused elsewhere
...
38. Two parallel nodes both write the SAME key. What happens?
    ...
    - B) Last write wins (overwrite) unless a reducer is defined
    - C) The graph crashes
...
39. The supervisor/multi-agent approach mainly helps with:
    ...
    - C) Breaking complex tasks into specialised agents
...
40. "Message passing" between agents is implemented in this stack by:
    ...
    - D) Passing messages through the shared graph state

## 5. Human-in-the-Loop

41. `interrupt()` inside a node:
    - A) Pauses the graph and returns control to the caller until resumed
    - B) Stops the process
    - C) Skips the node
...
42. The value you pass to `Command(resume=...)` becomes:
    - A) A system message
    - B) The return value of `interrupt()` in that node
...
46. The most sensible thing to gate with HITL:
    ...
    - B) Sending an email or executing a payment
    - C) Token counting
    - D) Model selection
...
47. If a human doesn't respond to an interrupt for a while:
    ...
    - C) State is checkpointed; you can resume the same thread later
...
49. The approve/reject decision in the guardrails HITL is made by:
    - A) A human review node acting on the human's response
    - B) The LLM by itself
    - C) The ToolNode
    - D) Random choice
...
50. "Graceful degradation" when the human rejects means:
    ...
    - B) The agent proceeds without the action and explains, or proposes an alternative
...
51. MCP's purpose is to give agents:
    ...
    - B) Access to a GPU
    - C) A standard way to expose/use external tools, resources, and prompts
...
52. `@mcp.prompt` registers:
    ...
    - B) A data resource
    - C) A vector index
    - D) A reusable prompt template clients can fetch

53. For a streamable_http MCP server, the client connects to:
    - A) The server's URL, e.g. `http://localhost:8000/mcp`
    - B) A file path
...
55. Tavily's MCP server is best described as:
    ...
    - C) A hosted remote server reached over HTTPS with your API key
...
56. In the shopping capstone, the agent obtains its tools by:
    ...
    - D) Calling `MultiServerMCPClient.get_tools()` on the books server

57. The books MCP server exposes price lookup through:
    - A) MCP tools plus a resource like `book://{book_id}`
    - B) A SQL table only
    - C) A REST API only
    - D) A CSV file
...
58. You would choose stdio transport when:
    ...
    - B) The server is a local subprocess of the same script
...
64. MCP tools differ from ordinary `@tool` functions because they are:
    ...
    - D) Fetched from the MCP server at runtime via `get_tools()`
...
66. Why split documents into chunks before embedding?
    ...
    - B) Embedding whole docs is expensive and retrieval becomes imprecise
...
67. If a chunk is too large, the typical problem is:
    ...
    - B) Nothing changes
    - C) Embedding quality and retrieval relevance drop, and context-window pressure grows
...
68. The retriever's output is used by the LLM as:
    ...
    - B) Training data
    - C) A checkpointer
    - D) Context in the prompt when generating the answer

69. Retrieval beats stuffing the entire document into the prompt because:
    - A) The model only gets the focused, relevant context
...
70. Why does MultiQueryRetriever improve recall?
    ...
    - B) Different question phrasings retrieve different relevant chunks
...
71. HyDE helps by:
    ...
    - C) Generating a hypothetical passage that bridges the query/document semantic gap
...
74. RAG reduces hallucination mainly by:
    ...
    - B) Grounding the answer in retrieved evidence
...
75. Retrieval keeps returning irrelevant chunks. The best first fix is:
    ...
    - B) Add more tools
    - C) Improve chunking/overlap, embeddings, or use multi-query retrieval
...
76. Deterministic (regex/rule) guardrails are usually applied first because:
    ...
    - D) They are cheap and fast

77. Model-based guardrails are preferred when:
    - A) The decision needs semantic understanding (is this content unsafe?)
...
78. An output guardrail runs:
    ...
    - B) After model generation, before the response reaches the user
    - C) During training
...
79. Redacting the email before the model means:
    ...
    - C) The raw PII never reaches the LLM call
...
80. Filtering a request like "how do I hack..." at input time is:
    ...
    - B) HITL
    - C) RAG retrieval
    - D) Input/harmful-content filtering

81. Middleware in `create_agent` wraps:
    - A) Model and tool calls — input flows through the layers before/after each
    - B) Only the chat UI
    - C) The vector store
    - D) The database
...
82. Layered guardrails (deterministic + model-based + HITL) are used because:
    ...
    - B) No single check is perfect — defence in depth
...
83. A guardrail false positive (safe input blocked) means:
    ...
    - C) User experience suffers — tune thresholds carefully
...
84. Guardrails can also validate tool-call arguments in order to:
    ...
    - D) Prevent risky or injected tool invocations

85. Prompt injection ("ignore your instructions") is best defended by:
    - A) Treating user input as untrusted data and filtering/validating it before it influences tool flows
...
86. A notebook fails with a localhost connection error. The first suspect is:
    ...
    - B) Ollama is not running / the model is not pulled
...
87. Where should your cloud API keys live in this repo?
    ...
    - C) In a repo-root `.env` file loaded via python-dotenv
...
88. Which of these needs cloud API keys in this repo?
    ...
    - C) None
    - D) The OpenAI RAG notebooks and the Tavily MCP clients

89. The Gradio UI talks to the shopping agent by:
    - A) Importing `get_shopping_agent` from `agent_shopping` and invoking it
...
90. Corporate SSL errors when installing/running typically mean:
    ...
    - B) The corporate proxy CA must be trusted (certifi/pyOpenSSL/python-certifi-win32)
